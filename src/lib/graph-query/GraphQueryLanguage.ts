...
export class GraphQueryLanguage extends EventEmitter {
  ...
  private parseQuery(queryString: string): ParsedQuery {
    ...
    if (nodeTypeMatch) {
      query.nodeType = nodeTypeMatch[2] as EntityType;
    }
   ...
   * Execute MATCH query
   */
  private async executeMatch(query: ParsedQuery): Promise<{ nodes: Entity[]; edges: Relationship[] }> {
    let nodes = this.graph.findEntities({});
    let edges: Relationship[] = [];
   ...
   */
  private async executeCreate(query: ParsedQuery): Promise<Entity[]> {
   ...
   */
  private async executeUpdate(query: ParsedQuery): Promise<Entity[]> {
  ...
  private async executeDelete(query: ParsedQuery): Promise<void> {
    ...
    for (const node of nodes) {
      this.graph.removeEntity(node.id);
    }
  }
   ...
   */
  private applyFilters(nodes: Entity[], filters: QueryFilter[]): Entity[] {
   ...
   */
  private matchesFilter(node: Entity, filter: QueryFilter): boolean {
   ...
   */
  private getFieldValue(node: Entity, field: string): any {
  ...
  private applyTraversal(
    ...
    const resultNodes = new Set<Entity>();
  ...
  private traverse(
    ...
    if (currentDepth < path.maxDepth) {
      const outgoing = this.graph.getRelationships(startNode.id, 'out');
      ...
      for (const edge of outgoing) {
        ...
        const targetNode = this.graph.getEntity(edge.target);
        if (targetNode) {
   ...
   */
  private applyOrdering(nodes: Entity[], orders: QueryOrder[]): Entity[] {
